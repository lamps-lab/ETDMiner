...
def extract_all_field(soup):
    ...
    advisor = None
    advisor_list = []
    advisor_elelments = soup.find_all('dim:field',{'qualifier':'chair'})
    ...
    # Degree
    degree = soup.find('dim:field',{'element':'degree', 'qualifier': 'name'})
    ...
    second_identifier = soup.find('dim:field',{'qualifier':'doi'})
...
def insertPDFs(soup, etdid, etdPath):
    ...
    urlInitials = 'https://ecommons.cornell.edu/bitstream/handle/'
    url = soup.find('dim:field',{'qualifier':'uri'})
    url = url.get_text()
