...
def getPDFdownloadUrl(soup):
    hrefValue = None # item-page-file-link'
    findClasses = soup.find_all('div', {'class':'item-page-file-link'})
    if findClasses:
        findClass = findClasses[1] # classWithDesiredLink
        if findClass:
            anchortag = findClass.find('a')
            if anchortag:
                hrefValue = anchortag['href']
                hrefValue = "https://open.bu.edu/" + hrefValue
...
for record in records:
    ...
    metadatas = tree.xpath('//uketd_dc:uketddc', namespaces={'uketd_dc': 'http://naca.central.cranfield.ac.uk/ethos-oai/2.0/'})
    print(metadatas)
