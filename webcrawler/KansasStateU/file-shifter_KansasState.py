...
def extract_all_field(soup):
    ...
    # Author @Dennis
    author = soup.find('dim:field',{'element':'contributor', 'qualifier': 'author'})
    ...
    # University
    university = soup.find('dim:field',{'qualifier':'publisher'})
    ...
    # Degree
    degree = soup.find('dim:field',{'qualifier': 'degree'})
...
def print_shifter_log(text):    
    with open("shifter_log.txt",'a',encoding = 'utf-8') as f:
        f.write(text+"\n")    
...
def main():    
    ...
    for etddir in etddirs:
        ...
        if os.path.exists(xmlFilePath):
            ...
            if is_exist:
                etdid = is_exist
                print_shifter_log(str(etdid) + "exist")
                ...
                print_shifter_log(str(etdid) + "new")
