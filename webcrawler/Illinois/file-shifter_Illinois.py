...
def extract_all_field(soup):
    ...
    # Advisor @Dennis
    advisor_list = []
    advisor_elelments = soup.find_all('dc:advisor')
    ...
    url = None
    url_list = []
    url_elements = soup.find_all('dc:identifier')
    if url_elements:
        for url_element in url_elements:
            if url_element.get_text().startswith('http://'):
                url = url_element.get_text()
                break   
    ...
    # University
    university = soup.find('dc:publisher')
...
def main():    
    ...
    for i in range(6):
        ...
        harvestDirectory = 'harvest1/2142/' + str(i)
        ...
        for etddir in etddirs:
            ...
            etdPath = os.path.join(harvestDirectory+'/'+etddir,etddir+'.pdf')
