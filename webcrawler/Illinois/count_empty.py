...
def count_empty_fields(column_name):
    ...
    query = f"SELECT COUNT(*) as count FROM etds WHERE {column_name} IS NULL AND university = 'University of Illinois at Urbana-Champaign'"
...
def count_total():        
    ...
    db_connection = mysql.connector.connect(**config)
    mycursor = db_connection.cursor(dictionary=True)
    query = f"SELECT COUNT(*) as count FROM etds WHERE university = 'University of Illinois at Urbana-Champaign' "
    mycursor.execute(query)
    result = mycursor.fetchone()
...
def count_has_no_pdf():
    db_connection = mysql.connector.connect(**config)
    mycursor = db_connection.cursor(dictionary=True)
    query = f"SELECT COUNT(*) as count FROM etds WHERE haspdf = 0 AND university = 'University of Illinois at Urbana-Champaign'"
    mycursor.execute(query)
    result = mycursor.fetchone()
