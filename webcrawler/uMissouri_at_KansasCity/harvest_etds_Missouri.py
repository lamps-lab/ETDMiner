...
sickle = Sickle('https://mospace.umsystem.edu/oai/request')
records = sickle.ListRecords(metadataPrefix='dim', set='com_10355_3987')
...
def getPDFdownloadUrl(soup):
    ...
    if findClass:
        anchortag = findClass.findAll('a')
        if anchortag:
            for tag in anchortag:
                if 'research.pdf' in tag['href']:
                    hrefValue = tag['href']
                    hrefValue = "https://mospace.umsystem.edu" + hrefValue
...
for record in records:
    ...
    for url in files:
        ...
        if 'hdl.handle.net' in etdLandingPage:
