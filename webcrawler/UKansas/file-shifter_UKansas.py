...
def insertETDs(soup):
    ...
    # University
    university = soup.find('dim:field',{'element':'publisher'})
    ...
    # Degree
    degree = soup.find('dim:field',{'qualifier':'degreeLevel', 'element':'thesis'})
    ...
    # Department
    department = soup.find('dim:field',{'element':'department'})
    if department is not None:
        department = department.get_text()

    # Discipline
    discipline = soup.find('dim:field',{'qualifier':'degreeDiscipline', 'element':'thesis'})
...
def main():    
    ...
    """
    harvestDirectory = 'harvest/1808'
    print(harvestDirectory)
    etddirs = os.listdir(harvestDirectory)
    etddirs = handleSuddenStop(etddirs,'19005') #TODO: Change here to handle sudden production stop   #metadc53494
