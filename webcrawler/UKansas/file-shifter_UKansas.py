...
def extract_all_field(soup):
    ...
    # Author @Dennis
    author_list = []
    author_elelments = soup.find_all('dim:field',{'element':'author'})
    ...
    # Landing Page URL
    URI = soup.find('dim:field',{'element':'identifier','qualifier':'uri'})
    if URI is not None:
        URI = URI.get_text()
    ...
    # Degree
    degree = soup.find('dim:field',{'element':'thesis','qualifier':'degreelevel'})
    if degree is not None:
        degree = degree.get_text()
    if degree is None:
        degree = soup.find('dim:field', {'element': 'type','mdschema':'dc'})
        if degree is not None:
            degree = degree.get_text()
    ...
    # Discipline
    discipline = soup.find('dim:field',{'qualifier':'degreeDiscipline'})
