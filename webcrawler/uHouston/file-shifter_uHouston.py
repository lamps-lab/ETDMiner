...
def insertETDs(soup):
    ...
    if author is not None:
        ...
        if author.find(',') != 1:
            ...
            author = ",".join(author)
        pullDigits = ''.join([char for char in author if not char.isdigit()])
        pullDigits = pullDigits.split('-')[0]
        author = pullDigits
    ...
    # Department
    department = soup.find('dim:field',{'qualifier':'department', 'element':'degree'})
...
def insertPDFs(soup, etdid, etdPath):
    ...
    urlInitials = 'https://uh-ir.tdl.org/bitstream/handle/'
    url = soup.find('dim:field',{'qualifier':'uri'})
    url = url.get_text()
...
def main():    
    ...
    """
    harvestDirectory = 'harvest/10657'
