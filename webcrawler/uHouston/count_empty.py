...
config = {
    ...
    #Changed databse name to 'test_db' for testing purposes
    'database': 'test_db' 
}
...
def count_empty_fields(column_name):
    ...
    query = f"SELECT COUNT(*) as count FROM etds WHERE {column_name} IS NULL AND university = 'Auburn University'"
...
def count_total():        
    ...
    db_connection = mysql.connector.connect(**config)
    mycursor = db_connection.cursor(dictionary=True)
    query = f"SELECT COUNT(*) as count FROM etds WHERE university = 'Auburn University' "
    mycursor.execute(query)
    result = mycursor.fetchone()
...
def count_has_no_pdf():
    db_connection = mysql.connector.connect(**config)
    mycursor = db_connection.cursor(dictionary=True)
    query = f"SELECT COUNT(*) as count FROM etds WHERE haspdf = 0 AND university = 'Auburn University'"
    mycursor.execute(query)
    result = mycursor.fetchone()
