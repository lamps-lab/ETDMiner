...
sickle = Sickle('https://scholarsbank.uoregon.edu/oai/request')
records = sickle.ListRecords(metadataPrefix='dim', set='com_1794_7555')
...
def getPDFdownloadUrl(soup):
    hrefValue = None
    findClass = soup.find('div', {'class':'file-wrapper clearfix'})
    if findClass:
        anchortag = findClass.find('a')
        if anchortag:
            hrefValue = anchortag['href']
            hrefValue = "https://scholarsbank.uoregon.edu" + hrefValue
