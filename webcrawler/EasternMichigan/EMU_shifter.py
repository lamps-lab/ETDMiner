...
def insertETDs(soup):
    ...
    # Landing Page URL
    url = soup.find('div',{'id':'doi'})
    if url is not None:
        url = url.find('p').get_text()
    ...
    # Discipline
    discipline = soup.find('div',{'id':'major'})
    ...
    mycursor = db_connection.cursor()
    oads_flag = 0
    sql = "INSERT INTO etds (title, author, advisor, year, university, URI, department, degree, discipline, language, abstract, oadsclassifier) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    val = (title, author, advisor, date, university, url, department, degree, discipline, language, abstract, oads_flag)
    mycursor.execute(sql, val)
    db_connection.commit()
