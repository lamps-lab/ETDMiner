...
sickle = Sickle('https://thescholarship.ecu.edu/oai/request')
records = sickle.ListRecords(metadataPrefix='dim', set='com_10342_122')
...
def getPDFdownloadUrl(soup):
    hrefValue = None
    findClass = soup.find('div', {'class':'col-sm-12 thumbnailSized first-item'})
    if findClass:
        anchortag = findClass.find('a')
        if anchortag:
            hrefValue = anchortag['href']
            hrefValue = "https://thescholarship.ecu.edu" + hrefValue
