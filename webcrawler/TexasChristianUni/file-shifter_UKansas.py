...
def extract_all_field(soup):
    ...
    # Advisor @Dennis
    advisor_list = []
    advisor_elelments = soup.find_all('dc:contributor')
    ...
    URI_list = soup.find_all('dc:identifier')
    if URI_list:
        for uri in URI_list:
            if 'http' in uri.text:
                URI = uri.text                
            elif 'etd' in uri.text:
                second_identifier = uri.text
        

    # Year
    date = soup.find_all('dc:date')
    if date and len(date)>=4:
        date = date[3].get_text()   
    ...
    # Degree
    degree = soup.find_all('dc:type')
    if degree:
        if len(degree)>=2:
            degree = degree[1].text
        else:
            degree = degree[0].text
    ...
    # Department
    department = soup.find('dc:department')
...
def main():    
    ...
    """
    harvestDirectory = 'harvest/116099117'
