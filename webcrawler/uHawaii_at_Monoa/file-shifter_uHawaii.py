...
def insertETDs(soup):
    ...
    # Landing Page URL
    url = soup.find('dim:field',{'qualifier':'uri', 'element':'identifier'})
    if url is not None:
        url = url.get_text()
    ...
    # Degree
    degree = soup.find('dim:field',{'qualifier':'degreelevel'})
    ...
    # Department
    department = soup.find('dim:field',{'qualifier':'department', 'element':'contributor'})
...
def insertPDFs(soup, etdid, etdPath):
    ...
    urlInitials = 'https://scholarspace.manoa.hawaii.edu/bitstream/handle/'
    url = soup.find('dim:field',{'qualifier':'uri', 'element':'identifier'})
    url = url.get_text()
...
def main():    
    ...
    """
    harvestDirectory = 'harvest/10125'
