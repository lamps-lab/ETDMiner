...
data = pd.read_csv("500ETD.csv", encoding = 'utf-8')
...
train_df.to_csv("train_original_58k.csv", encoding = 'utf-8', index = None)
val_df.to_csv("validation_original_19k.csv", encoding = 'utf-8', index = None)
test_df.to_csv("test_original_13k.csv", encoding = 'utf-8', index = None)

print("############## Splitting Completed and Saved !!!!!!! ####################")

train_df = pd.read_csv("train_original_58k.csv")
train_df['text'] = train_df['text'].astype(str)

val_df = pd.read_csv("validation_original_19k.csv")
val_df['text'] = val_df['text'].astype(str)

test_df = pd.read_csv("test_original_13k.csv")
test_df['text'] = test_df['text'].astype(str)

label_map = {"Label-Appendices": 0, "Label-ReferenceList": 1, "Label-Other": 2, "Label-TableofContent": 3,  "Label-TitlePage": 4, "Label-Abstract": 5, "Label-ListofFigures": 6, "Label-Acknowledgement": 7, "Label-ListofTables": 8, "Label-CurriculumVitae": 9, "Label-Dedication": 10, "Label-ChapterAbstract": 11, "Label-Chapters":12}
...
def create_multimodal_model(
    ...
    outputs = keras.layers.Dense(13, activation="softmax")(concatenated)
...
checkpoint = tf.keras.callbacks.ModelCheckpoint('multimodal_resNET50BERT.h5', monitor='val_loss', save_best_only=True, verbose=1)
...
#multimodal_model.load_weights('multimodal_resNET50BERT.h5')
...
def detailed_test_eval(model):
    ...
    result.to_csv('ETDSeg_confusion-matrix.csv')
    return result
