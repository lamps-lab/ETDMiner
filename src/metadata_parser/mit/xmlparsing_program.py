...
csvfile = open('program_xml.csv', 'w')
...
for filename in files:
    with open(filename, 'r', encoding="utf-8") as content:
        ...
        program_data=[]
        md_node = doc_root.find(".//dim:field[@element='subject']", prefix_map)
        if md_node is not None:
            program = md_node.text
            #print(program)
            #tokenize the sentence
            sent_tokens = sent_tokenize(program)
            #normalize the sentence
            sent_tokens = [sent.lower() for sent in sent_tokens]
            
            program_data.append(sent_tokens)
            #print(program_data)
            
    csv_writer.writerow(program_data)
