...
if __name__ == "__main__":
    ...
    df1.to_csv("tokenized_string_test.csv", encoding = 'utf-8', index = None)
    ...
    df4.to_csv("upper_left_test.csv", index = None)
    

    dfList=[]
    colname=['tokens']
    df5 = pd.read_csv("tokenized_string_test.csv", encoding='utf-8')
    dfList.append(df5)
    concatDf = pd.concat(dfList, axis =0)
    concatDf.columns=colname
    concatDf.to_csv("tokenized_string_test-updated.csv", index = None)
    
    df1List=[]
    colname=['left_margin']
    df6 = pd.read_csv("left_margin_test.csv", encoding='utf-8')
    df1List.append(df6)
    concatDf = pd.concat(df1List, axis =0)
    concatDf.columns=colname
    concatDf.to_csv("left_margin_test-updated.csv", index = None)
    
    df2List=[]
    colname=['bottom_right']
    df7 = pd.read_csv("bottom_right_test.csv", encoding='utf-8')
    df2List.append(df7)
    concatDf = pd.concat(df2List, axis =0)
    concatDf.columns=colname
    concatDf.to_csv("bottom_right_test-updated.csv", index = None)
    
    df3List=[]
    colname=['upper_left']
    df8 = pd.read_csv("upper_left_test.csv", encoding='utf-8')
    df3List.append(df8)
    concatDf = pd.concat(df3List, axis =0)
    concatDf.columns=colname
    concatDf.to_csv("upper_left_test-updated.csv", index = None)
    
    df9 = pd.read_csv("tokenized_string_test-updated.csv", encoding = 'utf-8')
    df10 = pd.read_csv("left_margin_test-updated.csv", encoding = 'utf-8')
    df11 = pd.read_csv("bottom_right_test-updated.csv", encoding = 'utf-8')
    df12 = pd.read_csv("upper_left_test-updated.csv", encoding = 'utf-8')
    
    result = pd.concat([df9, df10, df11, df12], axis = 1, sort = False)
    result.to_csv("visual_features_test.csv", encoding = 'utf-8', index = None)
